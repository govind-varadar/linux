...
static unsigned char
myrs_get_event(myrs_hba *cs, unsigned int event_num,
...
static int myrs_get_config(myrs_hba *cs)
{
...
static bool myrs_create_mempools(struct pci_dev *pdev, myrs_hba *cs)
{
	struct Scsi_Host *shost = cs->host;
...
static void myrs_destroy_mempools(myrs_hba *cs)
{
...
static void myrs_unmap(myrs_hba *cs)
{
	if (cs->event_buf) {
		kfree(cs->event_buf);
		cs->event_buf = NULL;
...
static void myrs_cleanup(myrs_hba *cs)
{
	struct pci_dev *pdev = cs->pdev;
...
static bool myrs_err_status(myrs_hba *cs, unsigned char status,

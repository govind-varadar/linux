...
static struct virtio_scsi_target_state *
virtscsi_get_target(struct virtio_scsi *vscsi, struct scsi_device *sdev,
		    int *target_id)
{
				 ...
				 struct scsi_cmnd *sc)
{
	...
	tgt = virtscsi_get_target(vscsi, sc->device, &target_id);
	if (!tgt) {
...
static int virtscsi_device_reset(struct scsi_cmnd *sc)
{
	...
	if (!virtscsi_get_target(vscsi, sc->device, &target_id))
		return FAST_IO_FAIL;
...
static int virtscsi_target_alloc(
	...
	if (rport) {
		...
		spin_unlock_irq(&vscsi->rescan_lock);
		if (!starget->hostdata) {
			dev_printk(KERN_WARNING, &starget->dev,
				   "target %d lookup failed\n", starget->id);
			return -ENODEV;
		}
...
static void virtscsi_rescan_work(struct work_struct *work)
{
	...
	struct virtio_scsi_target_state *tgt, *tmp, *old = NULL;
	DECLARE_COMPLETION_ONSTACK(comp);

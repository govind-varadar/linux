...
static void fdomain_finish_cmd(struct fdomain *fd, unsigned char host_byte)
{
	outb(0, fd->base + REG_ICTL);
	fdomain_make_bus_idle(fd);
	set_host_byte(fd->cur_cmd, host_byte);
...
static void fdomain_work(struct work_struct *work)
{
	...
	if (cmd->SCp.phase & in_arbitration) {
		...
		if (!(status & ASTAT_ARB)) {
			fdomain_finish_cmd(fd, DID_BUS_BUSY);
			goto out;
		}
		...
		if (!(status & BSTAT_BSY)) {
			...
			if (fdomain_select(cmd->device->host, scmd_id(cmd))) {
				fdomain_finish_cmd(fd, DID_NO_CONNECT);
				goto out;
			}
			/* Stop arbitration and enable parity */
			outb(ACTL_IRQEN | PARITY_MASK, fd->base + REG_ACTL);
...
static int fdomain_abort(struct scsi_cmnd *cmd)
{
	...
	fdomain_finish_cmd(fd, DID_ABORT);
	spin_unlock_irqrestore(sh->host_lock, flags);
	return SUCCESS;
}

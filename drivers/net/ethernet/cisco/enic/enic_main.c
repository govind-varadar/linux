	...
	case VNIC_DEV_INTR_MODE_MSIX:
		...
		for (i = 0; i < enic->qp_count; i++) {
			snprintf(enic->msix[i].devname,
				 sizeof(enic->msix[i].devname), "%s-qp-%u",
				 netdev->name, i);
...
static int enic_set_intr_mode(struct enic *enic)
{
	...
	BUG_ON(ARRAY_SIZE(enic->msix_entry) < enic->qp_count + 2);
	for (i = 0; i < enic->qp_count + 2; i++)
	...
	if (enic->config.intr_mode < 1) {
		/* TODO: use pci_alloc_irq_vectors. Below api is deprecated.
		 * not fixing it in this series for backbard compatibility
		 */
	...
	if (ENIC_SETTING(enic, RSS) &&
		...
		if (pci_enable_msix_range(enic->pdev, enic->msix_entry,
			...
			enic->cq_count = n + m;
			enic->intr_count = n + m + 2;
			enic->qp_count = max_t(unsigned int, enic->rq_count,
					       enic->wq_count);

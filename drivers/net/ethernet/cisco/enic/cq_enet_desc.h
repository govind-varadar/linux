...
(le16_to_cpu(desc->bytes_written_flags) & CQ_ENET_RQ_DESC_FLAGS_TRUNCATED)

#define CQ_DESC_PKT_LEN(desc)						\
(le16_to_cpu(desc->bytes_written_flags) & CQ_ENET_RQ_DESC_BYTES_WRITTEN_MASK)

#define CQ_DESC_RSS_HASH(desc)						\
(le32_to_cpu(desc->rss_hash))
...
#define CQ_DESC_VLAN_TCI(desc)						\
(le16_to_cpu(desc->vlan))
